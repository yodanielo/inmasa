  <div id="topbar-holder">
    <div id="topbar">
      ...
      <div id="search">
          <a id="btnaccesoclientes"><img src="<?php bloginfo('template_url'); ?>/images/btn-acceso-clientes.png"/></a>
        <form action="<?php bloginfo('url'); ?>/" method="get" id="searchform">
          ...
          <input type="submit" value="<?php echo trnslt('search'); ?>" class="submit cp" />
            ...
            <a id="linkcorreo" target="_blank" href="#"><img src="<?php bloginfo('template_url'); ?>/images/btn-acceso-correo.png"/></a>
      </div>
      <div id="grabber"></div>
    </div>
  </div>
  ...
  <div id="logo-container">
    ...
    $blogdesc="<br/>".get_option("blogdescription");

    if(trim($logoText) == '') { $logoText = get_bloginfo('name'); }
    if(trim($logoImg) != '') {
      $logoImg = makePathAbsolute($logoImg);
    ?>
      <a href="<?php bloginfo('url'); ?>"><img src="<?php echo $logoImg; ?>" alt="<?php echo $logoText; ?>" /><?=$blogdesc?></a>
    <?php
    } elseif($logoText != get_bloginfo('name')) {
    ?>
    <a href="<?php bloginfo('url'); ?>"><p style="margin:45px 0 0 0;"><?php echo $logoText; ?><?=$blogdesc?></p></a>
    <?php
    } else {
    ?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery("#menu-principal").children(":eq(0)").css("width","149px");
        jQuery("#menu-principal").children(":eq(1)").css("width","204px");
        jQuery("#menu-principal").children(":eq(2)").css("width","204px");
        jQuery("#menu-principal").children(":eq(3)").css("width","95px");
        jQuery("#menu-principal").children(":eq(4)").css("width","179px");
        jQuery("#menu-principal").children(":eq(5)").css("width","112px");
    });
</script>
  <div id="header-s"></div>
